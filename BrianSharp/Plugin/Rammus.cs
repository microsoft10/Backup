...
namespace BrianSharp.Plugin
{
    internal class Rammus : Helper
    {
        public Rammus()
        {
            ...
            E = new Spell(SpellSlot.E, 325, TargetSelector.DamageType.Magical);
        ...
        private void Clear()
        {
            ...
            if (minionObj.Count == 0 || Player.HasBuff("PowerBall"))
            {
                return;
            }
            if (GetValue<bool>("Clear", "Q") && Q.IsReady() && !Player.HasBuff("DefensiveBallCurl") &&
                (minionObj.Count(i => Q.IsInRange(i)) > 2 || minionObj.Any(i => i.MaxHealth >= 1200 && Q.IsInRange(i)) ||
                 minionObj.Count(i => Orbwalk.InAutoAttackRange(i, 40)) == 0) && Q.Cast(PacketCast))
            {
                return;
            }
            ...
            {
                var obj = minionObj.Find(i => E.IsInRange(i) && i.Team == GameObjectTeam.Neutral);
