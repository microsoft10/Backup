...
namespace DevTwitch
{
    class Program
    {
        ...
        public static void Combo()
        {
            ...
            var EnMin = Config.Item("eUlt").GetValue<Slider>().Value;
            var EnemiesList = ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsValid && !h.IsDead && h.Distance(Player.Position) <= 700 && h.IsEnemy).ToList();
			
			if (eTarget.IsValidTarget(Config.Item("qRange").GetValue<Slider>().Value) && Q.IsReady() && useQ)
            {
                Q.Cast();
            }
        ...
        private static void InitializeMainMenu()
        {
			...
			Config.SubMenu("Combo").AddItem(new MenuItem("qRange", "Q on range").SetValue(new Slider(1000, 1, 2000)));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseWCombo", "Use W").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseECombo", "Use E").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRCombo", "Use R Killable").SetValue(true));
			Config.SubMenu("Combo").AddItem(new MenuItem("eUlt", "Auto R when x enemies").SetValue(new Slider(3, 1, 5)));
