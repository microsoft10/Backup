...
namespace Irelia_Reloaded
{
    internal class Program
    {
        ...
        {
            var useQh = Menu.Item("useQHS").GetValue<bool>();
            var useWh = Menu.Item("useWHS").GetValue<bool>();
            var useEh = Menu.Item("useEHS").GetValue<bool>();
			var hMana = Menu.Item("HarassMana").GetValue<Slider>().Value;
            var target = TargetSelector.GetTarget(Q.Range, TargetSelector.DamageType.Physical);

            if (!target.IsValidTarget() || Player.ManaPercentage() < hMana)
            {
                return;
            }
                ...
                if (useWh && W.IsReady() && target.IsValidTarget(W.Range))
                {
                    W.Cast(Packets);
                }

                if (useQh && Q.IsReady() && !target.UnderTurret())
                {
                    Q.CastOnUnit(target, Packets);
                }
                                      
                if (useEh && E.IsReady() && target.IsValidTarget(E.Range))
                {
                    E.Cast(target, Packets);
                }
        ...
        private static void SetupMenu()
        {
			...
			// Harass
            var harassMenu = new Menu("Harass", "cmHarass");
            harassMenu.AddItem(new MenuItem("useQHS", "Harass With Q").SetValue(false));
			harassMenu.AddItem(new MenuItem("useWHS", "Harass With W").SetValue(true));
            harassMenu.AddItem(new MenuItem("useEHS", "Harass With E").SetValue(true));
			harassMenu.AddItem(new MenuItem("HarassMana", "Harass Mana %").SetValue(new Slider(20)));
