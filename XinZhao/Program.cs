...
namespace XinZhao
{
    internal class Program
    {
        ...
        public static void Combo()
        {
			...
			var EnMin = Config.Item("eForR").GetValue<Slider>().Value;
            var EnemiesList = ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsValid && !h.IsDead && h.Distance(Player.Position) < R.Range && h.IsEnemy).ToList();
			...
			if (R.IsReady() && EnemiesList.Count >= EnMin)if(R.IsReady() && EnemiesList.Count >= EnMin)
			{	
                R.CastOnUnit(rTarget);
                return;
            }
			
			if (rTarget != null && R.IsReady() && Config.Item("HPForR").GetValue<Slider>().Value >= ((Player.Health / Player.MaxHealth) * 100))
			{	
                R.CastOnUnit(rTarget);
                return;
            }
			
			if (Config.Item("useR").GetValue<bool>() && rTarget != null && R.IsReady() && GetComboDamage(rTarget) >= rTarget.Health + 50 && Player.Distance(rTarget) <= R.Range)
            {
                R.CastOnUnit(rTarget);
                return;
            }
        ...
        private static void CreateChampionMenu()
        {
			...
			Config.SubMenu("Combo").AddItem(new MenuItem("useR", "Use R Finish").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("HPForR", "Use R if my HP < X%").SetValue(new Slider(50, 100, 0)));
			Config.SubMenu("Combo").AddItem(new MenuItem("eForR", "Use R if enemy >").SetValue(new Slider(3, 5, 0)));
            Config.SubMenu("Combo")
                .AddItem(
